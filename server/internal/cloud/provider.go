// Package cloud uploads the backup bundle to a folder the user picked in
// their Dropbox or Google Drive, on a schedule they chose.
//
// The pieces are deliberately separated: `Provider` is everything
// account-specific (the OAuth dance, browsing folders, uploading bytes),
// `Service` is the provider-agnostic domain (the settings row, token refresh,
// when the next run is due), and `Scheduler` is the goroutine that ticks. Only
// this package talks to a third party over the network — the domain layer in
// internal/core stays pure SQL.
package cloud
...
// Provider IDs. These are the values the `provider` column and the wire
// contract carry, so they are frozen alongside the schema CHECK constraint.
const (
...
// native handle — a Drive file id, a Dropbox path — and is what gets stored
// as `folder_id`; Path is what the UI shows.
type Folder struct {
	ID   string `json:"id"`
...
type Provider interface {
	// ID is the stable wire identifier ("dropbox", "google_drive").
	ID() string
...
func NewRegistry(dropbox, google Credentials, client *http.Client, now func() time.Time) Registry {
	return Registry{
		NewDropbox(dropbox, client, now),
		NewGoogleDrive(google, client, now),
	}
}
...
// tokenResponse is the shape both providers' /token endpoints return.
