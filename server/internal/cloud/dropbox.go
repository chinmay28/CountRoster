...
// the bundle, and read the account so the UI can name whose Dropbox this is.
// `files.content.read` is deliberately absent — CountRoster never reads a
// bundle back out, and a token that can't download is a smaller loss.
...
// scheduled backup stop working the same afternoon it was set up.
...
// Upload writes the bundle into the chosen folder. `autorename` is on so two
// runs in the same second can't fail on a name clash — the schedule matters
// more than the exact filename.
