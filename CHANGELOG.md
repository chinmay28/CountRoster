...
straight out of this file when a `v*` tag (or a `release/v*` branch) is pushed.
A heading must name the tag exactly — the workflow refuses to publish a tag that
isn't the version its commit builds.
